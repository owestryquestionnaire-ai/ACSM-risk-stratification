...
def inject_custom_css():
    st.markdown(
        ...
        /* =========================================================
           📱 MOBILE RESPONSIVE PATCH (iPhone View)
           ========================================================= */
        @media (max-width: 768px) {
            ...
            .stRadio > div { gap: 1rem !important; padding-bottom: 10px !important;} /* Wider gap for fat fingers */

            /* 3. Make Buttons Mobile Friendly (Wrap text if too long) */
            button[kind="primary"], [data-testid="baseButton-primary"],
            button[kind="secondary"], [data-testid="baseButton-secondary"] {
                font-size: 18px !important;
                padding: 12px 10px !important; /* Taller for touch */
...
def render_inline_question(label, key, options=("否", "有"), check_error=False):
    ...
    with col1:
        if is_missing:
            # 移除 ❌，保留紅色高光
            st.markdown(
                f'<div class="question-text" style="color: #c62828; font-weight: bold; background-color: #ffebee; border-left: 4px solid #c62828; padding-left: 8px;">{label}</div>',
                unsafe_allow_html=True)
        else:
...
def tab_b_acsm(b_class, show_all_tabs):
    ...
    if b_class == "Pending":
        st.button("➡️ 儲存並前往下一步", type="primary", use_container_width=True, on_click=try_complete_b,
        ...
        if not show_all_tabs:
            ...
            c1, c2 = st.columns(2)
            with c1:
                st.button("✅ 完成運動風險判別（請交給職員）", type="primary", use_container_width=True,
                          on_click=try_complete_b, args=("3. Target HR & Clinical Guidelines",))
            with c2:
            ...
            st.button("➡️ 儲存並前往「2. 體能活動準備問卷」", type="primary", use_container_width=True,
                      on_click=try_complete_b, args=("2. 體能活動準備問卷 (表格 A)",))
    else:
        st.button("➡️ 儲存並前往「2. 體能活動準備問卷」", type="primary", use_container_width=True,
                  on_click=try_complete_b, args=("2. 體能活動準備問卷 (表格 A)",))
...
def tab_a_parq():
    ...
    st.button("✅ 完成運動風險判別（請交給職員）", type="primary", use_container_width=True, on_click=try_complete_a,
              args=("3. Target HR & Clinical Guidelines",))
